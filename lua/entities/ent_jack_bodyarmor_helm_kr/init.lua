...
function ENT:SpawnFunction(ply,tr)
	...
	local ent=ents.Create("ent_jack_bodyarmor_helm_kr")
	ent:SetPos(SpawnPos)
...
function ENT:Initialize()
	...
	self.Entity:SetMaterial("models/mat_jack_monotone_acu_flat")
	...
	self.Entity:SetColor(Color(137,146,124))
end
...
function ENT:Use(activator,caller)
	if(activator:IsPlayer())then
		if((not(activator.JackyArmor.Helmet))and(not(activator.JackyArmor.Suit)))then
			JackaBodyArmorUpdate(activator,"Helmet","Kevlar Resin",self:GetColor())
