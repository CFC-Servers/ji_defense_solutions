...
function EFFECT:Init(data)
	...
	local emitter = ParticleEmitter(vOffset)
		...
		for i=0,5*Scayul do
			...
			particle:SetVelocity(math.Rand(40,60)*VectorRand()*Scayul*i)
			particle:SetAirResistance(3000)
			particle:SetGravity(Vector(0, 0, math.Rand(25, 10000)))
			particle:SetLifeTime(0)
			particle:SetDieTime(math.Rand(0.01,1.5))
			particle:SetStartAlpha(59)
	...
	timer.Simple(0.025,function()
		...
		for i=0,10*Scayul do
			...
			local particle = Emitter:Add(sprite,vOffset)
			if(particle)then
				particle:SetVelocity(math.Rand(7,9)*VectorRand()*Scayul*i)
				particle:SetAirResistance(1000)
				particle:SetGravity(Vector(0,0,math.Rand(0,4000)))
	...
	if(dlight)then
		...
		dlight.Decay=15
		dlight.DieTime=CurTime()+6*Scayul
		dlight.Style=0
	end
